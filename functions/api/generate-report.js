// Cloudflare Pages Function: /api/generate-report
// 接收答题数据 → 调 DeepSeek → 写 D1 数据库 → 返回报告
...
export async function onRequest(context) {
  ...
  if (!DEEPSEEK_API_KEY) {
    return jsonResponse({ success: false, error: 'API Key 未配置' }, 500);
  }

  try {
    const { answers, dim_scores, total_score, level } = await request.json();
    ...
    if (!deepseekRes.ok) {
      ...
      return jsonResponse({ success: false, error: `DeepSeek 返回错误: ${deepseekRes.status}` }, 502);
    }
    ...
    if (!reportText) {
      return jsonResponse({ success: false, error: 'DeepSeek 返回了空内容' }, 502);
    }

    // 提取一句话总结（📊 你的 AI 能力画像 下面的第一行）
    ...
    if (db) {
      try {
        const result = await db.prepare(
          'INSERT INTO quiz_responses (answers, dim_scores, total_score, level) VALUES (?, ?, ?, ?)'
        ).bind(
        ...
        // D1 的 lastRowId 在 meta 里
        const responseId = result.meta?.last_row_id;
        if (responseId) {
          await db.prepare(
            'INSERT INTO quiz_reports (response_id, summary, report_html) VALUES (?, ?, ?)'
          ).bind(responseId, summary, reportText).run();
        }
      } catch (dbErr) {
        // 数据库写入失败不影响返回报告——前端仍然能展示
        console.error('D1 写入失败:', dbErr.message);
      }
    }
